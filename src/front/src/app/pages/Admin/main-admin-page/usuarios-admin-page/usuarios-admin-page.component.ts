...
export const prettyLog = (msg: string) => {
	console.log("%c %s", myCss, msg);
};
...
export class UsuariosAdminPageComponent implements OnInit {
	...
	toggleAlunoTable() {
		this.isShowingAlunoTable = true;
	}

	toggleProfessorTable() {
		this.isShowingAlunoTable = false;
	}
	...
	private handleProfessorResponse(response: ProfessorResponse[]) {
		this.professores = [];
		response.forEach((profResponse) => {
		...
		if (this.isShowingAlunoTable) {
			...
			return this.professores.map((professor) => {
				...
				return {
					...
					telefone: professor.usuario.numero,
					id: professor.id,
					tipo: professor.usuario.tipo,
					usuario: professor.usuario,
					status: professor.usuario.status,
					dataNasc: dataNascimento.toLocaleDateString("pt-BR"),
				};
			});
		}
