...
import { AlunoFiltro } from '../../../../models/aluno.model';

@Component({
  selector: 'app-fincanceiro-admin-page',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, BotaoComponent, ModalComponent, SimpleTableComponent, MultiSelectInputComponent, SearchBoxMultiComponent ],
...
export class FincanceiroAdminPageComponent {
  ...
  colunas = [
		...
		{ chave: 'criadoEm', titulo: 'Criado em', formatar:(valor: Date) => valor != null ? this.formartarData(valor) : '' },
    { chave: 'pagoEm', titulo: 'Pago em', formatar:(valor: Date) => valor != null ? this.formartarData(valor) : ''},
    { chave: 'mesesAtrasos', titulo: 'Meses Atrasados',width: '12%', formatar:(valor: number) => valor != null ? this.formatarAtraso(valor) : '' },
    { chave: 'valor', titulo: 'Valor', width: '12%', formatar:(valor: number) => valor != null ? this.formatarValor(valor) : '' },
  ...
  buscarAluno(termo: any) {
    ...
    this.adminService.filtrarAlunosNT(filtro).subscribe({
			next: (response) => {
